...
export class Transpiler {
	...
	transpile(sourceName, source) {
		...
		// workaround for issue where typescript ignores noResolve when resolveModuleNames is defined
		let hostResolveModuleNames = this._host.resolveModuleNames;
		this._host.resolveModuleNames = undefined;

		let program = ts.createProgram([sourceName], this._options, this._host);

		this._host.resolveModuleNames = hostResolveModuleNames;
