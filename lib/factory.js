System.register(['typescript', './logger', './compiler-host', './transpiler', './resolver', './type-checker', './format-errors', './utils', './libFiles'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var ts, logger_1, compiler_host_1, transpiler_1, resolver_1, type_checker_1, format_errors_1, utils_1, libFiles_1;
    var logger;
    ...
    function createServices(options, builder, _resolve, _lookup, _fetch) {
        ...
        if (options.typeCheck) {
            ...
            return addLibFilesToHost(host, _resolve, _fetch).then(function () { return ({ host: host, transpiler: transpiler, resolver: resolver, typeChecker: typeChecker, options: options }); });
        }
        return Promise.resolve({ host: host, transpiler: transpiler, resolver: resolver, typeChecker: typeChecker, options: options });
    }
    function addLibFilesToHost(host, _resolve, _fetch) {
        var allLibs = libFiles_1.getDefaultLibFilePaths(host.options).map(function (_a) {
            var libRef = _a[0], libFile = _a[1];
            return _resolve(libFile).then(function (libAddress) {
                return _fetch(libAddress).then(function (libSource) {
                    var file = host.addFile(libRef, libSource);
                    file.isLibFile = true;
    ...
    return {
        setters:[
            ...
            function (libFiles_1_1) {
                libFiles_1 = libFiles_1_1;
            }],
        execute: function() {
