...
export class IncrementalCompiler {
	constructor(fetch, resolve, options) {
		...
		this._options.resolveAmbientRefs = (this._options.resolveAmbientRefs !== false); 
		...
		if (!this._files[filename]) {
			...
			this._files[filename].loaded = this.fetch(filename)
				...
				.then((depsMap) => {
					...
					let text = this._files[filename].text;
					
					if (this._options.resolveAmbientRefs)
						text = this.replaceAmbientRefs(text, depsMap);
	...
	getDependencies(filename, info) {
		...
		let deps = info.referencedFiles.map((ref) => {
			if (isAmbientReference(ref.fileName) && !this._options.resolveAmbientRefs)
				return "./" + ref.fileName;
			else
				return ref.fileName;
		});
